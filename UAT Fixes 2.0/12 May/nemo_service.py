...
from app.services.rate_limiter import RateLimitExceeded as _RateLimitExceededType
from app.services.rate_limiter import UpstreamModelRateLimitError
...
async def _get_guardrail(
    ...
    1. If *guardrail_version_id* is provided, load the snapshot from
       ...
       from it (version-pinned execution).
    ...
    if environment == "prod":
        # Legacy fallback: resolve the latest active version for this guardrail
        from app.models.guardrail_version import GuardrailVersion as GVLegacy
        from sqlalchemy import select as _select
        stmt = (
            _select(GVLegacy)
            .where(
                GVLegacy.guardrail_id == guardrail_id,
                GVLegacy.is_active.is_(True),
            )
            .order_by(GVLegacy.version_number.desc())
            .limit(1)
        )
        ...
        if version_row:
            ...
            logger.info(
                "NeMo guardrail lookup succeeded (prod legacy via active version): "
                "guardrail_id=%s, version=v%d, name=%s",
                guardrail_id, version_row.version_number, row.name,
            )
            return row
        # Fall through to direct catalogue lookup if no active version
        logger.warning(
            "NeMo guardrail prod lookup: no active version found for guardrail_id=%s, "
            "falling back to catalogue draft.",
            guardrail_id,
        )
    ...
    logger.info(
        ...
        "guardrail_id=%s, resolved_id=%s, name=%s, model_registry_id=%s",
        guardrail_id, row.id, row.name, row.model_registry_id,
    )
    return row
...
async def _get_model_registry_config(
    ...
    # Coerce 0 -> None so "0 means unlimited" is honored consistently with the
    # canonical model-service columns.
    rpm_raw = getattr(model_row, "rate_limit_rpm", None)
    tpm_raw = getattr(model_row, "rate_limit_tpm", None)
    config: dict[str, Any] = {
        ...
        "rate_limit_rpm": rpm_raw if rpm_raw and rpm_raw > 0 else None,
        "rate_limit_tpm": tpm_raw if tpm_raw and tpm_raw > 0 else None,
    }
...
_DEFAULT_UPSTREAM_RETRY_AFTER_SECONDS = 30


def _classify_llm_call_exception(exc: BaseException) -> tuple[bool, int]:
    """Inspect an exception raised during rail LLM execution.

    Returns ``(is_rate_limit, retry_after_seconds)``. Walks the exception chain
    (``__cause__`` and ``__context__``) so it works on NeMo's
    ``LLMCallException`` wrappers that hold the original SDK exception.

    A 429 is detected from any of:
      * ``status_code`` / ``code`` / ``http_status`` attribute equal to 429
      * the OpenAI / Anthropic ``RateLimitError`` class name
      * substring match in the message: ``"Error code: 429"``,
        ``"rate_limit_exceeded"``, ``"too many requests"``

    ``Retry-After`` is parsed from ``exc.response.headers["retry-after"]``
    when present; otherwise falls back to a 30-second default.
    """
    seen: set[int] = set()
    current: BaseException | None = exc
    rate_limit_indicators = ("error code: 429", "rate_limit_exceeded", "too many requests")
    ...
    while current is not None and id(current) not in seen:
        ...
        for attr in ("status_code", "code", "http_status"):
            ...
            if value == 429 or value == "429":
                return True, _extract_retry_after(current)
        ...
        if any(token in message for token in rate_limit_indicators):
            return True, _extract_retry_after(current)
...
def _extract_retry_after(exc: BaseException) -> int:
    """Best-effort Retry-After header read off an SDK exception."""
    ...
    if headers is not None:
        ...
        if raw:
            try:
                ...
                pass
    return _DEFAULT_UPSTREAM_RETRY_AFTER_SECONDS
...
def _build_fallback_llm_params_from_error(
    ...
    if (
        _contains_all(normalized_error, ("unsupported", "parameter", "reasoningeffort"))
        or _contains_all(normalized_error, ("unsupported", "value", "reasoningeffort"))
    ) and "reasoning_effort" in params:
        params.pop("reasoning_effort", None)
        changes.append("removed:reasoning_effort")
        ...
        async def _compat_llm_call(  # type: ignore[override]
            ...
            if isinstance(params, dict):
                ...
                if (
                    ...
                    # "low" is supported across gpt-5, gpt-5.1, gpt-5.2, o1, o3.
                    # ("minimal" is gpt-5-only and is rejected by gpt-5.1.)
                    params["reasoning_effort"] = "low"
                    logger.info(
                        "NeMo llm_call adjusted: set reasoning_effort=low for reasoning-family model "
...
async def apply_nemo_guardrail_text(
    ...
    ``rate_limiter`` is an optional ``app.services.rate_limiter.RateLimiter``
    instance. When provided, the model's RPM/TPM is enforced against the same
    Redis bucket model-service uses, and ``RateLimitExceeded`` is raised on
    breach (the router converts that to 429).
    """
    ...
    try:
        ...
        guardrail = await _get_guardrail(
            session, guardrail_uuid, environment=environment, guardrail_version_id=guardrail_version_id
        )

        step = "lookup_model_registry"
        model_config = await _get_model_registry_config(session, guardrail)

        # Pre-flight RPM + TPM check. Skipped when no limiter is configured or
        # the model has no rate limits set. Raises RateLimitExceeded on breach,
        # which the router converts to HTTP 429 with Retry-After.
        if rate_limiter is not None and model_config:
            model_registry_id = model_config.get("model_registry_id")
            rpm = model_config.get("rate_limit_rpm")
            tpm = model_config.get("rate_limit_tpm")
            if model_registry_id is not None and (rpm or tpm):
                step = "rate_limit_check"
                rate_limit_ctx = await rate_limiter.enforce(
                    model_registry_id, rpm=rpm, tpm=tpm,
                )
        ...
        try:
            ...
            is_rate_limit, retry_after = _classify_llm_call_exception(exc)
            if is_rate_limit:
                raise UpstreamModelRateLimitError(
            ...
            try:
                ...
                is_rate_limit, retry_after = _classify_llm_call_exception(inner_exc)
                if is_rate_limit:
                    raise UpstreamModelRateLimitError(
                        provider=(model_config or {}).get("provider"),
                        model=(model_config or {}).get("model_name"),
                        retry_after_seconds=retry_after,
                        original_message=str(inner_exc),
                    ) from inner_exc
                raise
        ...
        # Post-flight TPM debit. ``debit_tpm`` is a no-op when the context is
        # None or has no TPM limit, so this is safe to call unconditionally.
        if rate_limit_ctx is not None:
            try:
                total = int(llm_usage.get("total_tokens", 0) or 0)
                if total > 0:
                    await rate_limit_ctx.debit_tpm(total)
            except Exception:
                logger.exception(
                    "NeMo guardrail TPM debit failed: "
                    f"guardrail_id={guardrail_id}, total_tokens={llm_usage.get('total_tokens')}"
                )
    ...
    except _RateLimitExceededType:
        # Rate-limit denial is expected platform behavior, not an execution
        # failure -- log as WARN and re-raise so the router emits a 429.
        elapsed_ms = (perf_counter() - started_at) * 1000
        logger.warning(
            "NeMo guardrail rate-limited: "
            f"guardrail_id={guardrail_id}, step={step}, duration_ms={elapsed_ms:.2f}"
        )
        raise
    except UpstreamModelRateLimitError as exc:
        elapsed_ms = (perf_counter() - started_at) * 1000
        logger.warning(
            "NeMo guardrail upstream-rate-limited: "
            f"guardrail_id={guardrail_id}, step={step}, provider={exc.provider}, "
            f"model={exc.model}, retry_after_seconds={exc.retry_after_seconds}, "
            f"duration_ms={elapsed_ms:.2f}"
        )
        raise
    except Exception:  # noqa: BLE001
