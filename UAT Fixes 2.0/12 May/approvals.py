...
class GuardrailPromotionResult(BaseModel):
    ...
    prod_guardrail_id: str | None = None  # kept for legacy compat
...
async def _promote_guardrails_for_deployment(
    ...
    for node in snapshot.get("nodes", []):
        ...
        try:
            ...
            result.guardrail_version_id = promo.get("guardrail_version_id")
            result.version_number = promo.get("version_number")
            # Keep prod_guardrail_id for backwards compat (same as guardrail_version_id)
            result.prod_guardrail_id = result.guardrail_version_id
            result.in_sync = promo.get("in_sync", False)
            result.ready = True
            logger.info(
                "[GUARDRAIL_PROMOTION] Guardrail promoted on approval: "
                f"uat_id={guardrail_id}, version_id={result.guardrail_version_id}, "
                f"version=v{result.version_number}, in_sync={result.in_sync}"
            )
    ...
    *,
    ...
    if deployment.agent_snapshot:
        try:
            ...
            # Store guardrail version refs in the deployment snapshot for version-pinned execution
            if guardrail_promotions:
                version_refs = [
                    {
                        "guardrail_id": gp.uat_guardrail_id,
                        "guardrail_version_id": gp.guardrail_version_id,
                        "version_number": gp.version_number,
                    }
                    for gp in guardrail_promotions
                    if gp.ready and gp.guardrail_version_id
                ]
                if version_refs:
                    snapshot = deployment.agent_snapshot or {}
                    snapshot["guardrail_version_refs"] = version_refs
                    deployment.agent_snapshot = snapshot
                    await session.commit()
                    logger.info(
                        "[GUARDRAIL_PROMOTION] Stored %d guardrail version ref(s) on deployment %s",
                        len(version_refs), deployment.id,
                    )
    ...
    if model_req is not None:
        ...
        return ApprovalPreviewResponse(
            ...
            snapshot={
                ...
                "rate_limit_rpm": getattr(row, "rate_limit_rpm", None),
                "rate_limit_tpm": getattr(row, "rate_limit_tpm", None),
            },
        )
    assert req is not None
    deployment = await session.get(AgentDeploymentProd, req.deployment_id)
