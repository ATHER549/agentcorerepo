...
-- Partitions are automatically expired after 12 hours via TTL.
...
-- Create events_full table - primary write target with full I/O and metadata
...
-- Create events_core table - lightweight version with truncated I/O for fast list queries
-- Populated via materialized view from events_full.
...
(
    ...
    -- I/O (truncated to 200 chars via materialized view)
    input String,
    input_length UInt64 MATERIALIZED lengthUTF8(input),
    output String,
    output_length UInt64 MATERIALIZED lengthUTF8(output),

    -- Metadata (values truncated to 200 chars via materialized view)
    metadata_names Array(String),
    metadata_values Array(String),
...
-- Materialized view to populate events_core from events_full with truncated I/O
