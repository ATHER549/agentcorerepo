...
from app.services.rate_limiter import RateLimitExceeded, UpstreamModelRateLimitError
...
async def apply_guardrail(
    ...
    try:
        result = await nemo_service.apply_nemo_guardrail_text(
            ...
            guardrail_version_id=body.guardrail_version_id,
        )
        ...
        raise HTTPException(
            ...
            detail={
                ...
                "limit_type": exc.limit_type.value,
                "limit": exc.limit,
        ...
        raise HTTPException(
            status_code=429,
            detail={
                "error": "guardrail_model_rate_limit",
                "provider": exc.provider,
                "model": exc.model,
                "retry_after_seconds": exc.retry_after_seconds,
                "message": (
                    f"The model used by this guardrail "
                    f"({exc.provider}/{exc.model}) is rate-limited. "
