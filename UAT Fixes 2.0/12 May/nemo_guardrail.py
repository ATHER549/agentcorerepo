...
class NemoGuardrailComponent(Node):
    ...
    def _pre_run_setup(self):
        ...
        self._rate_limit_exc: GuardrailRateLimitError | None = None
    ...
    async def _evaluate_guardrail_decision(self) -> dict[str, Any]:
        if getattr(self, "_rate_limit_exc", None) is not None:
        ...
        # Detect production context — prod agents use version-pinned guardrail execution
        environment: str | None = None
        guardrail_version_id: str | None = None
        if self._vertex:
            is_prod = bool(getattr(self._vertex.graph, "prod_deployment_id", None))
            if is_prod:
                # Look up guardrail_version_refs from the deployment snapshot
                snapshot = getattr(self._vertex.graph, "raw_graph_data", None) or {}
                version_refs = snapshot.get("guardrail_version_refs") or []
                for ref in version_refs:
                    if ref.get("guardrail_id") == guardrail_id and ref.get("guardrail_version_id"):
                        guardrail_version_id = ref["guardrail_version_id"]
                        break
                if not guardrail_version_id:
                    # Legacy fallback — use environment="prod" for pre-migration deployments
                    environment = "prod"
                logger.info(
                    "NeMo guardrail node detected production context: "
                    f"guardrail_id={guardrail_id}, version_id={guardrail_version_id}, "
                    f"prod_deployment_id={getattr(self._vertex.graph, 'prod_deployment_id', None)}"
                )
        ...
        except GuardrailRateLimitError as exc:
            # Rate-limit failures must abort the agent run with a red error
            # bubble (matching `ModelRateLimitError` behavior), not appear
            # as a regular chat message. Logging the execution first so
            # dashboards still pick it up, then re-raising so the orchestrator
            # (rabbitmq/direct) error handler renders it as `category="error"`.
            logger.warning(
                "NeMo guardrail node rate-limited: "
                f"guardrail_id={guardrail_id}, error_code={exc.error_code}, "
                f"provider={exc.provider}, model={exc.model}, "
                f"retry_after_seconds={exc.retry_after_seconds}"
            )
            decision["blocked"] = True
            decision["action"] = "rate_limited"
            decision["blocked_text"] = exc.message
            decision["guardrail_id"] = guardrail_id
            decision["status"] = (
                f"Guardrail rate-limited ({exc.error_code}, retry after "
                f"{exc.retry_after_seconds}s)"
            )
            self._decision = decision
            self._decision_evaluated = True
            self._rate_limit_exc = exc
