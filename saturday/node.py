...
class Node(ExecutableNode):
    ...
    async def _build_with_tracing(self):
        ...
        try:
            ...
            # Tracing infrastructure failure — fall back to building without tracing
            from loguru import logger
            logger.warning(f"Tracing failed for {self.trace_name}, falling back to build without tracing: {e}")
            results, artifacts = await self._build_results()
