...
PINECONE_API_TIMEOUT = 60  # seconds — applies to query, upsert, embed, rerank
...
class PineconeVectorStoreNode(LCVectorStoreNode):
    ...
    def _ensure_index_exists(self):
        ...
        MAX_WAIT_ITERATIONS = 15  # 15 × 2s = 30s max
        for attempt in range(MAX_WAIT_ITERATIONS):
            try:
                ...
                if desc.status and desc.status.get("ready", False):
                    logger.info(f"[Pinecone] Index '{self.index_name}' ready after ~{attempt * 2}s")
                    break
            except Exception as e:
                logger.warning(f"[Pinecone] describe_index attempt {attempt + 1}: {e}")
            time.sleep(2)
        else:
            raise TimeoutError(
                f"Pinecone index '{self.index_name}' not ready after {MAX_WAIT_ITERATIONS * 2}s. "
                "Check the Pinecone dashboard for index status."
            )
    ...
    def _ingest_documents(self, documents, embedder):
        ...
        # Batch dense embeddings to avoid OOM on large document sets
        EMBED_BATCH_SIZE = 96
        dense_embeddings = []
        for i in range(0, len(texts), EMBED_BATCH_SIZE):
            batch = texts[i:i + EMBED_BATCH_SIZE]
            dense_embeddings.extend(embedder.embed_documents(batch))
        ...
        for i in range(0, len(vectors), 100):
            index.upsert(vectors=vectors[i:i+100], namespace=self.namespace or "", timeout=PINECONE_API_TIMEOUT)
        return len(vectors)
    ...
    def search_documents(self) -> list[Data]:
        ...
        try:
            count = self._ingest_if_needed(wrapped)
            if count > 0:
                logger.info(f"[Pinecone] Ingested {count} document(s) before search")
        except Exception as e:
