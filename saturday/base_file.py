...
class BaseFileNode(Node, ABC):
    ...
    def _download_from_azure(self, blob_path: str) -> Path:
        ...
        try:
            ...
            blob_size_mb = (props.size or 0) / (1024 * 1024)
            if blob_size_mb > MAX_BLOB_SIZE_MB:
                raise ValueError(
                    f"Blob '{blob_path}' is {blob_size_mb:.1f} MB, exceeding the "
                    f"{MAX_BLOB_SIZE_MB} MB limit."
                )

            logger.info(f"[BaseFile] Downloading blob '{blob_path}' from container '{container_name}' ({blob_size_mb:.1f} MB)...")
            downloader = blob_client.download_blob()

            if blob_size_mb > STREAM_THRESHOLD_MB:
                # Stream large files directly to disk to avoid OOM
                logger.info(f"[BaseFile] Streaming large blob ({blob_size_mb:.1f} MB) to disk")
                ...
                logger.info(f"[BaseFile] Streamed to temp: {local_path}")
                return local_path
