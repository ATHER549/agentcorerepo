...
class GeminiOCRExtractorNode(Node):
    ...
    MAX_PDF_PAGES = 200
    LLM_VISION_TIMEOUT = 120  # seconds per page/image
    ...
    def _extract_file(self, path: Path) -> list[Data]:
        # Reject files larger than MAX_FILE_SIZE_MB to prevent OOM
        file_size_mb = path.stat().st_size / (1024 * 1024)
        if file_size_mb > self.MAX_FILE_SIZE_MB:
            raise ValueError(
                f"File '{path.name}' is {file_size_mb:.1f} MB, exceeding the "
                f"{self.MAX_FILE_SIZE_MB} MB limit. Split the file and retry."
            )
    ...
    def _extract_pdf(self, path: Path) -> list[Data]:
        ...
        if total_pages > self.MAX_PDF_PAGES:
            logger.warning(
                f"[OCR] PDF '{path.name}' has {total_pages} pages, "
                f"truncating to first {self.MAX_PDF_PAGES} pages"
            )
    ...
    def _llm_vision_extract(self, image_bytes: bytes, mime_type: str) -> str | None:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            ...
            try:
                response = future.result(timeout=self.LLM_VISION_TIMEOUT)
            except concurrent.futures.TimeoutError:
                raise TimeoutError(
                    f"LLM vision OCR timed out after {self.LLM_VISION_TIMEOUT}s. "
                    "Check LLM connectivity or reduce document size."
                )
