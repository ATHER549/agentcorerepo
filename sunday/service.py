...
class TracingService(Service):
        ...
        try:
            while trace_context.running or not trace_context.traces_queue.empty():
                try:
                    ...
                    continue
                try:
                    # Run trace function in thread with timeout to prevent worker from hanging
                    await asyncio.wait_for(
                        asyncio.to_thread(trace_func, *args),
                        timeout=30.0,
                    )
                except asyncio.TimeoutError:
                    logger.warning(f"[Tracing] Trace function {getattr(trace_func, '__name__', 'unknown')} timed out after 30s")
    ...
    async def _stop(self, trace_context: TraceContext) -> None:
        try:
            ...
            # Drain remaining items with timeout to prevent deadlock
            if not trace_context.traces_queue.empty():
                try:
                    await asyncio.wait_for(trace_context.traces_queue.join(), timeout=10.0)
                except asyncio.TimeoutError:
                    logger.warning("[Tracing] Queue drain timed out after 10s — forcing shutdown")
            if trace_context.worker_task:
                trace_context.worker_task.cancel()
                try:
                    await asyncio.wait_for(trace_context.worker_task, timeout=5.0)
