...
async def generate_agent_events(
    *,
    ...
    async def _build_vertex(vertex_id: str, graph: Graph, event_manager: EventManager) -> VertexBuildResponse:
     ...
     # Track completed and in-progress vertices to prevent duplicate builds
    completed_vertices: set[str] = set()
    building_vertices: set[str] = set()
    build_lock = asyncio.Lock()
    BUILD_TIMEOUT = 600  # 10 minutes max for entire build
    ...
    async def build_vertices(
        ...
        """
        # Deduplicate: skip if already building or built
        async with build_lock:
            if vertex_id in building_vertices or vertex_id in completed_vertices:
                return
            building_vertices.add(vertex_id)
        ...
        if vertex_build_response.valid and vertex_build_response.next_vertices_ids:
            ...
            for next_vertex_id in vertex_build_response.next_vertices_ids:
                ...
                async with build_lock:
                    if next_vertex_id in building_vertices or next_vertex_id in completed_vertices:
                        continue
    ...
    try:
        ...
        # Only start non-input vertices whose predecessors are ALL already completed.
        # Vertices with unbuilt predecessors will be started recursively by
        # build_vertices after their predecessors complete. This prevents
        # duplicate builds and ensures correct dependency ordering.
        if non_input_ids:
            root_ids = []
            for vid in non_input_ids:
                predecessors = graph.predecessor_map.get(vid, [])
                if all(pred in completed_vertices for pred in predecessors):
                    root_ids.append(vid)

            if root_ids:
                tasks = [
                    asyncio.create_task(build_vertices(vid, graph, event_manager))
                    for vid in root_ids
                ]
                try:
                    await asyncio.wait_for(asyncio.gather(*tasks), timeout=BUILD_TIMEOUT)
                except asyncio.TimeoutError:
                    logger.error(f"Build timed out after {BUILD_TIMEOUT}s")
                    event_manager.on_error(data={
                        "errorMessage": f"Build timed out after {BUILD_TIMEOUT}s. "
                        "One or more components may be unresponsive."
                    })
                    ...
                    try:
                        failed_vertex = graph.get_vertex(root_ids[-1])
                        custom_component = failed_vertex.custom_component if failed_vertex else None
        ...
        # CRITICAL: Always end traces and push termination signal.
        # Without this, the SSE streaming response hangs forever on error,
        # freezing the connection and blocking the server.
        try:
            if graph is not None:
                await graph.end_all_traces()
        except Exception as e:
            logger.warning(f"Error ending traces during cleanup: {e}")
        # Always push termination signal so the SSE stream can close
